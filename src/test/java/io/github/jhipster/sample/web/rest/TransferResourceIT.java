...
class TransferResourceIT {
    ...
    private static final BigDecimal DEFAULT_AMOUNT = new BigDecimal("500.00");
    private static final String DEFAULT_DESCRIPTION = "Test transfer";
    ...
    void initTest() {
        ...
        sourceAccount = new BankAccount().name("Source Account").balance(new BigDecimal("1000.00")).user(testUser);
        sourceAccount = bankAccountRepository.saveAndFlush(sourceAccount);

        destinationAccount = new BankAccount().name("Destination Account").balance(new BigDecimal("500.00")).user(testUser);
    ...
    void executeTransferSuccessfully() throws Exception {
        ...
        // Validate transfer count increased
        ...
        assertThat(updatedSource.getBalance()).isEqualByComparingTo(new BigDecimal("500.00"));
        assertThat(updatedDest.getBalance()).isEqualByComparingTo(new BigDecimal("1000.00"));
    }
    ...
    void transferWithNonExistentSourceAccount() throws Exception {
        Transfer transfer = new Transfer()
            ...
            .sourceAccount(new BankAccount().id(Long.MAX_VALUE))
            .destinationAccount(new BankAccount().id(destinationAccount.getId()));
