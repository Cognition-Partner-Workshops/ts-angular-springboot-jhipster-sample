...
export class InvoiceService {
  ...
  update(invoice: IInvoice): Observable<EntityResponseType> {
    return this.http.put<IInvoice>(`${this.resourceUrl}/${encodeURIComponent(this.getInvoiceIdentifier(invoice))}`, invoice, {
      observe: 'response',
    });
  }

  partialUpdate(invoice: PartialUpdateInvoice): Observable<EntityResponseType> {
    return this.http.patch<IInvoice>(`${this.resourceUrl}/${encodeURIComponent(this.getInvoiceIdentifier(invoice))}`, invoice, {
      observe: 'response',
    });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IInvoice>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' });
  }
