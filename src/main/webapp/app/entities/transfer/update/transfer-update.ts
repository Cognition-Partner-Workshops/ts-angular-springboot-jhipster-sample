...
@Component({
  ...
  imports: [TranslateDirective, TranslateModule, NgbModule, FontAwesomeModule, AlertError, ReactiveFormsModule, DecimalPipe],
})
export class TransferUpdate implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.editForm.get('sourceAccount')?.valueChanges.subscribe(selectedSource => {
      this.updateDestinationAccounts(selectedSource);
    });
  }
  ...
  protected updateDestinationAccounts(selectedSource: IBankAccount | null): void {
    if (selectedSource) {
      this.filteredDestinationAccounts.set(this.bankAccounts().filter(a => a.id !== selectedSource.id));
    } else {
    ...
    if (currentDest && currentDest.id === selectedSource?.id) {
