...
export class TransferService {
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<ITransfer>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' });
  }
  ...
  queryByAccount(accountId: number, req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<ITransfer[]>(`${this.resourceUrl}/by-account/${encodeURIComponent(accountId)}`, {
      params: options,
      observe: 'response',
    });
  }
