...
public class TransferResource {
    ...
    /**
     * {@code GET  /transfers} : get all transfers for current user.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of transfers in body.
     */
    @GetMapping("")
    public ResponseEntity<List<Transfer>> getAllTransfers(@org.springdoc.core.annotations.ParameterObject Pageable pageable) {
        LOG.debug("REST request to get a page of Transfers");
        Page<Transfer> page = transferService.findAllForCurrentUser(pageable);
    ...
    /**
     * {@code GET  /transfers/by-account/:accountId} : get recent transfers for a specific account.
     *
    ...
    public ResponseEntity<List<Transfer>> getTransfersByAccount(
        @PathVariable("accountId") Long accountId,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get Transfers for account : {}", accountId);
        List<Transfer> transfers = transferRepository.findRecentByAccountId(accountId, pageable);
