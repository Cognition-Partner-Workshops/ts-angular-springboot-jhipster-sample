...
public class TransferResource {
    ...
    public ResponseEntity<Transfer> createTransfer(@Valid @RequestBody Transfer transfer) throws URISyntaxException {
        ...
        transfer = transferService.executeTransfer(transfer);
        return ResponseEntity.created(new URI("/api/transfers/" + transfer.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, transfer.getId().toString()))
            .body(transfer);
    }
    ...
    public ResponseEntity<?> getAllTransfers(Pageable pageable) {
        ...
        Page<Transfer> page = transferRepository.findAllForCurrentUser(pageable);
    ...
    public ResponseEntity<Transfer> getTransfer(@PathVariable("id") Long id) {
        ...
        // Filter: only return if user owns source or destination account
        transfer = transfer.filter(t -> {
            String currentLogin = SecurityUtils.getCurrentUserLogin().orElse("");
            boolean ownsSource =
                t.getSourceAccount() != null &&
                t.getSourceAccount().getUser() != null &&
                currentLogin.equals(t.getSourceAccount().getUser().getLogin());
            boolean ownsDestination =
                t.getDestinationAccount() != null &&
                t.getDestinationAccount().getUser() != null &&
                currentLogin.equals(t.getDestinationAccount().getUser().getLogin());
